...
public class SwerveModuleWrapper {
    ...
    public SwerveModuleWrapper(String name, int canID0, int canID1, int canIDAzimuth, double offsetAzimuth,
                               double location_x, double location_y) 
    {
        m_name = name;
        Falcon500Configuration falconConfig = new Falcon500Configuration(makeFalcon500Config(false, 101.0));

        SwerveMotor motors[] = new SwerveMotor[] { 
        ...
        config.set("differential-matrix",
                List.of(List.of(0.041666666, -0.041666666), List.of(0.069444444, 0.09722222)));
        ...
        m_swerveModule = new SwerveModule(motors, azimuthSensor, swerveModuleConfig);
    }
    ...
    public SwerveModuleState getState() {
        // getWheelVelocity returns speed in feet per second but SwerveModuleState wants
        // meters per second
        // WrappedAngle.asDouble returns angle in degrees
        SwerveModuleState swerveModuleState = new SwerveModuleState(
                Units.feetToMeters(m_swerveModule.getWheelVelocity()),
                Rotation2d.fromDegrees(m_swerveModule.getAzimuthPosition().asDouble()));
    ...
    public void setDesiredState(SwerveModuleState desiredState) {
        ...
        m_swerveModule.set(wheelVelocity, azimuthPosition);
    }

    // Populate NetworkTables with the current setttings. Use the TJ^2 system.
    public void populateNetworkTable() {
        SwerveModuleData data = new SwerveModuleData(m_swerveModule);
        NetworkTable mainTable = NetworkTableInstance.getDefault().getTable("swerveLibrary");

        data.populateNetworkTable(mainTable.getSubTable("modules").getSubTable(m_name));
    }
}
